...
pub async fn is_server_running() -> Result<Option<String>> {
    ...
    match timeout(Duration::from_secs(SERVER_PROBE_TIMEOUT_SECS), TcpStream::connect(&addr)).await {
        Ok(Ok(_)) => {
...
pub async fn wait_for_server(max_attempts: u32) -> Result<String> {
    debug!("Waiting for server to become ready (max {} attempts)", max_attempts);
    ...
    for attempt in 1..=max_attempts {
        ...
        debug!("Server not ready yet (attempt {}), waiting {}ms", attempt, SERVER_READY_RETRY_INTERVAL_MS);
...
pub fn spawn_client_process(
    ...
    let child = Command::new(command)
        ...
        .spawn()
        .map_err(|e| {
            SetuError::Other(format!("Failed to start {}: {}", command, e))
        })?;
...
impl ProcessManager {
    ...
    pub fn start_client_with_env(&mut self, command: &str, args: &[String], env_vars: &[(&str, &str)]) -> Result<()> {
