...
impl ModelRouter {
    ...
    pub fn route_model(&self, model_name: &str) -> Result<Vec<RoutingDecision>> {
        ...
        if let Some(model_route) = self.config.routing.models.get(model_name) {
            let fallback_models = match model_route {
                ModelRoute::Single(model) => vec![model.clone()],
                ModelRoute::Multiple(models) => models.clone(),
            };

            // Convert each fallback model to a routing decision
            let mut routing_decisions = Vec::new();
            for fallback_model in fallback_models {
                // Route each fallback model through the name-based router
                match self.name_based_router.route_model(&fallback_model) {
                    Ok(mut decision) => {
                        // Override the model name with the target model
                        decision.model = self.extract_model_name(&fallback_model);
                        decision.original_model = model_name.to_string();
                        routing_decisions.push(decision);
                    }
                    Err(e) => {
                        tracing::warn!("Failed to route fallback model '{}': {}", fallback_model, e);
                        continue;
                    }
                }
            }
...
mod tests {
    use super::*;
    use crate::config::{AuthConfig, ProviderConfig, RetryConfig, ServerConfig, RoutingConfig};
    ...
    fn test_extract_model_name() {
        ...
        assert_eq!(router.extract_model_name("anthropic/claude-3-5-sonnet"), "claude-3-5-sonnet");
        assert_eq!(router.extract_model_name("openrouter/z-ai/glm-4.5:fireworks"), "z-ai/glm-4.5");
