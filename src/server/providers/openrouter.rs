...
pub async fn handle_openrouter_request_from_openai(
        ...
        let openrouter_message = match message.role {
            ...
            ai_ox_common::openai_format::MessageRole::Assistant => {
                let mut assistant_msg = if let Some(content) = message.content {
                    ...
                    openrouter_ox::message::AssistantMessage::new(Vec::<
                        openrouter_ox::message::ContentPart,
                    >::new())
                };
                ...
                if let Some(tool_calls) = message.tool_calls {
                    assistant_msg.tool_calls = Some(
                        tool_calls
                            .into_iter()
                            .map(|tc| openrouter_ox::response::ToolCall {
                                index: None,
                                id: Some(tc.id),
    ...
    if let Some(req_str) = crate::server::error_handling::prepare_request_log(&openrouter_request) {
        tracing::debug!(target: "setu::request", "Outgoing OpenRouter (from OpenAI) request: {}", req_str);
    }
    ...
    match openrouter_client.send(&openrouter_request).await {
        Ok(response) => {
            if let Some(resp_str) = crate::server::error_handling::prepare_response_log(&response) {
                tracing::debug!(target: "setu::response", "OpenRouter response: {}", resp_str);
            }
