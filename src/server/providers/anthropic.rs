...
pub async fn handle_direct_anthropic_request(
    ...
    match anthropic_client.send(&anthropic_request).await {
        ...
        Err(e) => {
            let compacted_request = super::super::error_handling::compact_request_for_logging(&anthropic_request);
            tracing::error!("Failed streaming request (compacted): {}", compacted_request);
...
pub async fn create_anthropic_client(config: Arc<Mutex<Config>>, prefer_oauth: bool) -> Result<Anthropic, SetuError> {
    if prefer_oauth {
        ...
        if let Some(anthropic_provider) = config_guard.providers.get_mut("anthropic") {
            match AnthropicOAuth::validate_auth_config(&mut anthropic_provider.auth).await {
                Ok(()) => {
                    if let Ok(access_token) = AnthropicOAuth::get_valid_access_token(&mut anthropic_provider.auth, false).await {
                        info!("🔐 Anthropic → OAuth (subscription billing)");
                        return Ok(Anthropic::builder().oauth_token(&access_token).build());
                    }
                }
                Err(e) => {
                    tracing::warn!("OAuth authentication failed, falling back to API key: {}", e);
                }
            }
        }
    }
