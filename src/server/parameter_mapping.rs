...
pub fn apply_openrouter_parameters(
    ...
    apply_param(&mut request.frequency_penalty, query_params, "frequency_penalty");
    apply_param(&mut request.presence_penalty, query_params, "presence_penalty");
    apply_param(&mut request.repetition_penalty, query_params, "repetition_penalty");
    apply_param(&mut request.top_k, query_params, "top_k");
    ...
    // Reasoning flag
    let reasoning_flag = query_params.get("reasoning")
...
pub fn create_gemini_thinking_config(query_params: &HashMap<String, String>) -> Option<gemini_ox::generate_content::ThinkingConfig> {
    let thinking_budget = query_params.get("think")
        .and_then(|v| v.parse::<i32>().ok())
        .unwrap_or(0);
        
    let include_thoughts = query_params.get("thoughts")
        .and_then(|v| v.parse::<bool>().ok())
        .unwrap_or(false);
...
mod tests {
    ...
    fn test_provider_preferences() {
        ...
        assert_eq!(modified_prefs.sort, Some(openrouter_ox::provider_preference::Sort::Throughput));
        assert_eq!(modified_prefs.quantizations, Some(vec![openrouter_ox::provider_preference::Quantization::Int8]));
    }
