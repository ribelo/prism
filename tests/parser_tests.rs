...
fn test_parse_valid_model_formats() {
    ...
    let test_cases = vec![
        ...
        ("provider/complex-model-name-v2", "provider", "complex-model-name-v2"),
    ];

    for (input, expected_provider, expected_model) in test_cases {
        let parsed = ParsedModel::parse(input).unwrap();
        assert_eq!(parsed.provider, expected_provider, "Failed for input: {}", input);
...
fn test_parse_invalid_model_formats() {
    ...
    for invalid_input in invalid_cases {
        ...
        assert!(error_msg.contains("provider/model"), 
                "Error message should mention format requirement for: '{}'", invalid_input);
    }
}
...
fn test_parse_complex_model_names() {
    ...
    let complex_cases = vec![
        ("openrouter/anthropic/claude-3-5-sonnet-20241022", "openrouter", "anthropic/claude-3-5-sonnet-20241022"),
        ("huggingface/microsoft/DialoGPT-medium", "huggingface", "microsoft/DialoGPT-medium"),
        ("custom/org/model-v1.2.3-fine-tuned", "custom", "org/model-v1.2.3-fine-tuned"),
    ];

    for (input, expected_provider, expected_model) in complex_cases {
        let parsed = ParsedModel::parse(input).unwrap();
        assert_eq!(parsed.provider, expected_provider, "Failed for complex input: {}", input);
        assert_eq!(parsed.model, expected_model, "Failed for complex input: {}", input);
    }
}
...
fn test_parse_with_whitespace_trimming() {
    ...
    for (input, expected_provider, expected_model) in whitespace_cases {
        let parsed = ParsedModel::parse(input).unwrap();
        assert_eq!(parsed.provider, expected_provider, "Failed for whitespace input: {}", input);
        assert_eq!(parsed.model, expected_model, "Failed for whitespace input: {}", input);
    }
}